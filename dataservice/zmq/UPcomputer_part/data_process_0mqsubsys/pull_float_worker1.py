...
def pull_worker(context, url, sync_addr, exp_id_server, topic, exp_id):
    ...
    print("we have connected to the mysql")
    ...
    # process monitoring
    sock_monitor_url = "tcp://127.0.0.1:8009"
    ...
    # # Initialize poll set
    poller = zmq.Poller()
    poller.register(socket_sub_pull, zmq.POLLIN)
    ...
    while True:
        ...
        if socks.get(socket_sub_pull) == zmq.POLLIN:

            # 接收xpub的资料，其中已经经过了子系统的筛选
            b = socket_sub_pull.recv()
            counter+=1
            print('msg we receive', counter)
...
if __name__ == '__main__':
    ...
    exp_id = 0
    # 启动该进程对该子系统中的数据进行处理
    pull_worker(context, url, sync_addr, exp_id_server, sub_content[1], exp_id)
