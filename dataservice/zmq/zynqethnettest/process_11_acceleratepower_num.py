...
def daemon_thread(context):
    ...
    daemon_zmqaddr = "tcp://192.168.127.200:9001"
    daemon_zmq.connect(daemon_zmqaddr)
    daemon_zmq.setsockopt(zmq.RCVTIMEO,2000)


    d
    process_thread=threading.Thread(target=processerfuc,
                                    args=(context,))




def processerfuc(context,topic):
    ...
    #process monitoring
    sock_monitor_url = "tcp://192.168.127.200:9000"
    sock_process_monitor=context.socket(zmq.DEALER)
    sock_process_monitor.setsockopt(zmq.IDENTITY, b'process_11')
    ...
    while True:
        ...
        if socks.get(sock_process_monitor) == zmq.POLLIN:
            x=sock_process_monitor.recv()
            print('Can we receive something',x)
            if x==b'process alive?':
                sock_process_monitor.send(b'process yes')
        ...
        if socks.get(saverep) == zmq.POLLIN:
            ...
            print('Can we receive something', b)
            if b == b'process alive?':
                print('we can in herer')
                saverep.send(b'process yes')
            elif b==b'start saving':
                ...
                if t1!=None:
                    ...
                    t1 = threading.Thread(target=datasavethread, args=(context, datalist, savingpubaddr))
                    flagtosave = True
                ...
                saverep.send(b'start saving')
            elif b == b'stop saving':
                print(t1)
                stop_thread(t1)
                flagtosave = False
                t1=None
                print('we have thie thread')
                saverep.send(b'stop saving')
